  ...
  module Acts #:nodoc:
    module Taggable #:nodoc:
      ...
      module SingletonMethods
        ...
        alias_method :tags, :tag_counts

        def owner_tags(space)
          Tag.group('tags.id').joins(sanitize_sql(["INNER JOIN #{Tagging.table_name} ON `tags`.id = #{Tagging.table_name}.tag_id AND #{Tagging.table_name}.taggable_type = #{quote_value(base_class.name)}"]))
        end
